...
raw_writefile(Data *d, u8 min_pos) {
    ...
    u8 *buffers[MAXNUM/2+1] = {NULL};
    int err = 0;
    u8 f[BUFSIZE], s[BUFSIZE];
    while (d->comm.size) {
        size_t chunksize = d->comm.size < BUFSIZE ? d->comm.size : BUFSIZE;
#pragma omp parallel shared(err, buffers) private(f, s)
        {
#pragma omp for nowait schedule(static)
            for (u8 i = 0; i < d->comm.n/2 + 1; ++i) {
                if (fread(f, 1, chunksize, d->files[2*i]) != chunksize) {
                    err = -1;
                }
                if (2*i+1 < d->comm.n) {
                    if (fread(s, 1, chunksize, d->files[2*i+1]) != chunksize) {
                        err = -1;
                    }
                    xorv(f, s);
                }
                buffers[i] = f;
#pragma omp flush(buffers)
            }
#pragma omp master
            {
                for (u8 i = 1; i < d->comm.n/2 + d->comm.n%2; ++i) {
                    while (buffers[i] == NULL) {
#pragma omp flush(buffers)
                    }
                    xorv(buffers[0], buffers[i]);
                    buffers[i] = NULL;
                }
                if (fwrite(buffers[0], 1, chunksize, d->input) != chunksize) {
                    err = -1;
                }
#pragma omp flush(buffers)
            }
